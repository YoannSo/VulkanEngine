...
namespace lve {
	...
	{
		m_descriptorSetLayout = LveDescriptorSetLayout::Builder().addBinding(0, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT)
	...
	void GBuffer::createDescriptorSets()
	{
		...
		for (uint32_t i = 0; i < imageCount; i++) {
			...
			auto imageInfos = getDescriptorImageInfos(i);

			LveDescriptorWriter(*m_descriptorSetLayout, SceneManager::getInstance()->getPool())

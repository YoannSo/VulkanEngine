...
void LveCamera::setViewDirection(glm::vec3 position, glm::vec3 direction, glm::vec3 up) {
    const glm::vec3 w{ glm::normalize(direction) };// orthonormal basis, normalize, use it to construct rotation matrix, 
...
void LveCamera::setViewTarget(glm::vec3 position, glm::vec3 target, glm::vec3 up) {
    setViewDirection(position, target - position, up);//direction = targetr-position, 
}

void LveCamera::setViewYXZ(glm::vec3 position, glm::vec3 rotation) {//use inverse of the rotation matrix with the translation back to orifgin, inverse bc want to rotate to the current cam orientation (opposiote of model transform) (rotation matrix inverse = transpose)

...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  createInstance(); // create vulkan instance, init vilkan lib, 
  setupDebugMessenger();//setup validation layer, check for error
  createSurface();//create surface, connection between the window and vulkan
  pickPhysicalDevice();//pick the gpu, 
  createLogicalDevice();//describe what figure of our device we want to use
...
void LveDevice::createBuffer(//create buffer
    VkDeviceSize size,
    VkBufferUsageFlags usage,
    VkMemoryPropertyFlags properties, // take size usage and properties and return buffer and buffer memory
    VkBuffer &buffer,
    VkDeviceMemory &bufferMemory) {
  VkBufferCreateInfo bufferInfo{};//first creatinfo struct so set some member, how we use this buffer
  ...
  VkMemoryRequirements memRequirements;//querry the buffer memory requirement to allocate the mem
  ...
  vkBindBufferMemory(device_, buffer, bufferMemory, 0);//if all goof we bind the buffer
}

...
namespace lve {
	...
	void LvePipeline::defaultDeferredPipelineConfigInfo(PipelineConfigInfo& configInfo)
	{
	   ...
	   /* //viewport describe the transformation between pipeline output and target image gl=>output image
		...
		for (auto& attachment : configInfo.colorBlendAttachments) {
			...
			attachment.srcColorBlendFactor = VK_BLEND_FACTOR_ONE;   // facultatif si blendEnable = VK_FALSE
			attachment.dstColorBlendFactor = VK_BLEND_FACTOR_ZERO;  // facultatif
		...
		configInfo.colorBlendInfo.logicOp = VK_LOGIC_OP_COPY;  // facultatif
		...
		configInfo.colorBlendInfo.blendConstants[0] = 0.0f;  // facultatif
	...
	void LvePipeline::enableAlphaBlinding(PipelineConfigInfo& configInfo)
	{
		...
		for (auto& colorBlendInfo : configInfo.colorBlendAttachments) {
			colorBlendInfo.blendEnable = VK_TRUE;
			colorBlendInfo.colorWriteMask =
				VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT |
				VK_COLOR_COMPONENT_A_BIT;
			colorBlendInfo.srcColorBlendFactor = VK_BLEND_FACTOR_SRC_ALPHA;   // Optional
			colorBlendInfo.dstColorBlendFactor = VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;  // Optional

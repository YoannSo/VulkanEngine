...
namespace engine {
	...
	public:
		...
		ShadowRenderSystem(VkRenderPass renderPass, std::vector<VkDescriptorSetLayout>& p_descriptorSetLayout);//globalSetLayout to tell the pipeline what descriptor set layout will be
		~ShadowRenderSystem();

		ShadowRenderSystem(const ShadowRenderSystem&) = delete; // delete copy constructor
		ShadowRenderSystem& operator=(const ShadowRenderSystem&) = delete; // and copy operator, make sure to not have 2 pointer to window, then if we destroy one, the second isnt destroyed
